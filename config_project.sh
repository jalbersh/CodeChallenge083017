...
pushd ../${gitproject}/src/main/java/com/dish/ofm/service/codeChallenge083017/config
mv CodeChallenge083017Config.java ${pascalcase}Config.java
popd

pushd ../${gitproject}/src/main/java/com/dish/ofm/service
mv CodeChallenge083017Application.java ${pascalcase}Application.java
popd

pushd ../${gitproject}/src/main/java/com/dish/ofm/service/codeChallenge083017/controller
mv CodeChallenge083017Controller.java ${pascalcase}Controller.java
popd
echo -e "${Green}Renamed java files...${NC}"

pushd ../${gitproject}/src/main/java/com/dish/ofm/service
mv codeChallenge083017 ${pkgname}
popd
echo -e "${Green}Renamed source Directory to ${pkgname}...${NC}"

pushd ../${gitproject}/src/test/java/com/dish/ofm/service/codeChallenge083017/controller
mv CodeChallenge083017ControllerTest.java ${pascalcase}ControllerTest.java
popd
...
mv code_challange_083017_server.rb ${rubyfriendly}_server.rb
...
do
    sed -i.sedTmp \
    -e s/code-challenge-083017/${gitproject}/g \
    -e s/Code Challenge 083017/"${userfriendly}"/g \
    -e s/9999/"${simulatorport}"/g \
    -e s/8888/"${portnumber}"/g \
    -e s/challenge/"${restendpoint}"/g \
    -e s/challange 803017/"${restsuccess}"/g \
    -e s/challenge/"${restsuffix}"/g \
    -e s/CodeChallenge083017/"${pascalcase}"/g \
    -e s/codeChallenge083017/"${pkgname}"/g \
    -e s/CODE_CHALLENGE_083017/"${uppercased}"/g \
    -e s/code_challange_083017/"${rubyfriendly}"/g \
    $file
done
